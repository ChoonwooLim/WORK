...
**[2026.04 v2.2 신규] 🎬 Wan 2.2 AI 영상 생성 통합**: Alibaba의 공식 오픈소스 비디오 모델 **Wan 2.2**를 Orbitron 대시보드 내에서 직접 사용할 수 있습니다. 3종 variant를 모두 통합: `TI2V-5B`(5B, 텍스트+이미지→영상, ~36s/클립), `T2V-A14B`(MoE A14B, 플래그십 텍스트→영상), `I2V-A14B`(MoE A14B, 정지 이미지→영상). 대시보드 사이드바의 `AI 스튜디오 → 🎬 AI 영상 생성 (Wan 2.2)` 메뉴에서 프롬프트/이미지를 입력하면 MP4가 즉시 재생됩니다. 외부 API 비용 0원, 토큰당 과금 없음. (자세한 것은 [AI 영상 생성 문서](public/docs-content/ai-video.md) 참고)

**[2026.04 v2.2 신규] 🖥 분산 GPU 추론 아키텍처**: 대시보드 프로덕션 호스트와 AI 추론을 담당하는 **전용 GPU 서버(Threadripper 3970X + RTX 3090 24GB + 128GB RAM)**를 분리 운영합니다. 무거운 워크로드(31B LLM, A14B 비디오 MoE, Flux 이미지 생성)를 전용 머신으로 오프로드하여 대시보드 재시작이 진행 중인 AI 작업을 끊지 않습니다. 환경변수(`OLLAMA_HOST`, `WAN_VIDEO_HOST`)만 바꾸면 GPU 서버를 여러 대로 확장 가능. 한 대의 GPU에서 Gemma LLM + Wan 비디오 + Flux 이미지 서비스가 공존해도 OOM이 나지 않도록 **3중 VRAM 안전망**(Ollama 30초 idle 자동 언로드, Wan 서비스의 로드 직전 Ollama 언로드 훅, diffusers sequential CPU offload)이 내장되어 있습니다. 모든 통신은 Orbitron의 authMiddleware를 거친 서버사이드 프록시(`/api/wan/*`)로 이루어져 LAN IP가 브라우저에 유출되지 않고 비로그인 유저는 접근 불가합니다.
...
- **분산 GPU 아키텍처 + AI 영상 생성 (2026.04.13, v2.2)**:
  ...
  - **Wan 2.2 AI 영상 생성 서비스**: `/srv/wan-video-service/`에 FastAPI + diffusers 0.37.1로 3종 모델(TI2V-5B/T2V-A14B/I2V-A14B, 총 ~268GB) 호스팅. 대시보드의 `AI 스튜디오` 사이드바 섹션 + `/ai-video` 페이지 + 서버사이드 프록시 `/api/wan/*` (LAN IP 미노출). 테스트 결과: ti2v-5b는 17프레임 36s, A14B는 4-5분/클립.
  - **동적 VRAM 공유 3중 안전망**: (1) Ollama `OLLAMA_KEEP_ALIVE=30s`로 사용 중일 때만 VRAM 점유, (2) Wan 서비스가 A14B 로드 직전 Ollama 즉시 언로드 호출, (3) diffusers `enable_sequential_cpu_offload()` + `PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True`로 28GB 모델을 24GB GPU에 수용.
