...
## 빌드 성능 최적화 (2026.04 신규)

> ✨ **2026.04 업데이트**: Docker 빌드 캐싱이 기본 활성화되어 배포 속도가 **2~3배 향상**됩니다.

*   **레이어 캐싱**: `package.json`이 변경되지 않았다면 `npm install` 단계를 건너뛰고, 소스 코드 변경분만 빠르게 반영합니다.
*   **클린 빌드 옵션**: 캐시 문제가 의심될 때는 환경변수에 `DOCKER_NO_CACHE=true`를 추가하면 전체 리빌드를 수행합니다.
*   **60분 타임아웃**: 모든 배포에는 안전 타임아웃이 적용됩니다. 네트워크 장애 등으로 빌드가 멈추더라도 60분 후 자동 중단됩니다.
