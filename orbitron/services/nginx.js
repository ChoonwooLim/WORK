...
class NginxService {
    ...
    // Generate an HTTPS server block for a Let's Encrypt-secured custom domain.
    // Paths are the ones visible inside the dev-nginx container (/etc/letsencrypt/... via volume mount).
    _httpsBlock(domain, upstreamHost, upstreamPort) {
        const certDir = `/etc/letsencrypt/live/${domain}`;
        return `
# HTTPS (Let's Encrypt) for custom domain: ${domain}
server {
    listen 443 ssl;
    http2 on;
    server_name ${domain};
...
`;
    ...
    generateConfig(project, targetContainer) {
        ...
        // Check whether we can add an HTTPS block for the custom domain (needs a live cert)
        let httpsBlock = '';
        if (project.custom_domain) {
            try {
                const le = require('./letsencrypt');
                if (le.hasCert(project.custom_domain)) {
                    httpsBlock = this._httpsBlock(project.custom_domain, upstreamHost, upstreamPort);
                }
